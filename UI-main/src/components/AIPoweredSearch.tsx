...
const AIPoweredSearch: React.FC<AIPoweredSearchProps> = ({ 
  ...
  const featureDescriptions: Record<string, string[]> = {
    search: [
      'AI Powered Search',
      'Ask questions and search across your Confluence content using AI.'
    ],
    video: [
      'Video Summarizer',
      'Summarize and extract insights from video content in your pages.'
    ],
    code: [
      'Code Assistant',
      'Analyze, refactor, and generate code with AI assistance.'
    ],
    impact: [
      'Impact Analyzer',
      'Compare page versions and analyze the impact of changes.'
    ],
    test: [
      'Test Support Tool',
      'Generate test strategies and Q&A for your code documentation.'
    ],
    image: [
      'Image Insights',
      'Extract insights and build charts from images in your pages.'
    ]
  };
          ...
          {showSidebar && (
            ...
            >
              ...
              {features.map((feature) => {
                ...
                const desc = featureDescriptions[feature.id] || [feature.label, ''];
                return (
                  <div key={feature.id} className="relative group mb-2">
                    ...
                    <span className="absolute left-full top-1/2 -translate-y-1/2 ml-2 px-3 py-2 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap text-left min-w-[180px]">
                      <span className="block font-semibold">{desc[0]}</span>
                      <span className="block mt-1 text-xs font-normal">{desc[1]}</span>
                    </span>
                  </div>
                );
              })}
