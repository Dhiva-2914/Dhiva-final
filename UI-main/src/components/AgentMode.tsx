    ...
    try {
      ...
      for (let i = 0; i < instructions.length; i++) {
        ...
        if (toolForInstruction === 'test_support' && matchedPages.length === 2 && /test\s*strategy|generate\s*test/i.test(instruction)) {
          ...
          const formattedOutput = formatTestSupportOutput({ strategy: res.test_strategy || res.ai_response || '' });
        ...
        // Only trigger Impact Analyzer if explicitly requested and two pages are present
          ...
          const formattedOutput = formatImpactAnalyzerOutput({
            metrics: {
            ...
            riskLevel: {
              level: res.risk_level || 'low',
              score: res.risk_score || 0,
              factors: res.risk_factors || []
            },
            impactSummary: res.impact_analysis,
            diffResults: res.diff
          });
        ...
        // Special handling for sequential code actions and impact analysis
          ...
          // Format output using the tool formatter
          const formattedOutput = formatCodeAssistantOutput(outputs);
          ...
          for (const page of matchedPages) {
            ...
            if (toolForInstruction === 'ai_powered_search') {
              ...
              const formattedOutput = formatAIPoweredSearchOutput(output);
              if (!pageResults[page]) pageResults[page] = { tool: toolLabel, outputs: [], formattedOutput: '' };
              ...
              const formattedOutput = formatVideoSummarizerOutput({
                name: page,
                summary: output,
              ...
              const formattedOutput = formatTestSupportOutput({ strategy: output });
              if (!pageResults[page]) pageResults[page] = { tool: toolLabel, outputs: [], formattedOutput: '' };
              pageResults[page].outputs.push(output);
              pageResults[page].formattedOutput = formattedOutput;
              ...
              const formattedOutput = formatImageInsightsOutput([{
                name: page,
                summary: output
              }]);
              if (!pageResults[page]) pageResults[page] = { tool: toolLabel, outputs: [], formattedOutput: '' };
              ...
              if (!pageResults[page]) pageResults[page] = { tool: toolLabel, outputs: [], formattedOutput: '' };
              pageResults[page].outputs.push(output);
              pageResults[page].formattedOutput = output;
            }
          }
        }
      }
      ...
      const pageTabs = Object.keys(pageResults).length > 0 ? [
        {
          ...
          icon: FileText,
          content: '',
          results: Object.entries(pageResults).map(([page, { tool, outputs, formattedOutput }]) => ({ page, tool, result: outputs.join('\n\n'), formattedOutput })),
        }
      ] : [];
      const tabs = [
        ...(impactTab ? [impactTab] : []),
                          ...
                          {activeTab === 'impact-analysed' ? (
                            ...
                            <div>
                              ...
                              {activeResult && activeResult.type === 'page' && (
                                <div className="mt-4">
                                  {(() => {
                                    const result = (outputTabs.find(t => t.id === 'per-page-results')?.results || []).find((r: { page: string, tool: string, formattedOutput: string }) => r.page === activeResult.key);
                                    if (!result) return null;
                                    
                                    // Display the formatted output based on the tool type
                                    const tool = result.tool;
                                    const formattedOutput = result.formattedOutput;
                                    
                                    if (tool === 'code_assistant') {
                                      // Display code assistant output with proper formatting
                                      return (
                                        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                                          <div className="whitespace-pre-wrap text-gray-700" dangerouslySetInnerHTML={{ __html: formattedOutput.replace(/\n/g, '<br/>').replace(/```js\n([\s\S]*?)\n```/g, '<pre class="bg-gray-900 text-gray-300 p-4 rounded-lg overflow-auto"><code>$1</code></pre>') }} />
                                        </div>
                                      );
                                    } else if (tool === 'ai_powered_search') {
                                      // Display AI powered search output
                                      return (
                                        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                                          <div className="whitespace-pre-wrap text-gray-700">{formattedOutput}</div>
                                        </div>
                                      );
                                    } else if (tool === 'video_summarizer') {
                                      // Display video summarizer output
                                      return (
                                        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                                          <div className="whitespace-pre-wrap text-gray-700">{formattedOutput}</div>
                                        </div>
                                      ...
                                      return result.outputs?.map((output: string, index: number) => {
                                        // Check if this is Code Assistant output and contains code
                                        if (tool === 'code_assistant' && (output.includes('AI Action Output:') || output.includes('Target Language Conversion Output:') || output.includes('Modification Output:') || output.includes('Processed Code:'))) {
